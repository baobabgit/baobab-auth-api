# Cahier des charges — `baobab-auth-api` — v1.0.0

**Projet :** `baobab-auth-api`  
**Version cible :** `v1.0.0`  
**Titre :** Version stable, compatibilité inter-briques validée et API publique figée  
...
Publier une version stable de l’API d’authentification Baobab, avec contrats HTTP gelés, intégrations directes validées, documentation complète et niveau de qualité suffisant pour être utilisée par des APIs métiers.
...
Les dépendances directes de `baobab-auth-api` sont `baobab-auth-core`, `baobab-auth-database` et `baobab-auth-security`. La version `v1.0.0` de l’API doit valider explicitement les versions suivantes avant d’être considérée comme terminée.

| Librairie dépendante directe | Version à valider | Pourquoi cette validation est obligatoire |
|---|---:|---|
| `baobab-auth-core` | `v1.0.0` | Valider l’API métier stable : entités, ports, DTO, exceptions, policies, catalogue RBAC. |
| `baobab-auth-database` | `v1.0.0` | Valider persistance stable, migrations, repositories, transactions, performances de base. |
| `baobab-auth-security` | `v1.0.0` | Valider cryptographie stable, JWT/JWKS, refresh tokens, rotation, révocation, adaptateurs core. |
...
|---|---:|---|
| `baobab-auth-client` | `v1.0.0` | Validation finale obligatoire du client contre l’API stable. |
| `baobab-auth-admin` | `v1.0.0` | Validation finale des opérations admin compatibles. |
| `baobab-auth-service` | `v1.0.0` | Validation finale du déploiement complet API + DB. |

Règle de poursuite : une librairie dépendante ne doit pas poursuivre vers son jalon suivant si l’intégration prévue avec `baobab-auth-api` `v1.0.0` échoue sur un contrat bloquant.
...
- Contrats HTTP publics figés.
- Compatibilité validée avec toutes les briques `baobab-auth` en v1.0.0.
- Documentation utilisateur et développeur complète.
- Guide de déploiement via `baobab-auth-service`.
- Politique de versionnement sémantique.
- Changelog final et tag release.
- Tests qualité, sécurité et intégration complets.

## 4. Endpoints concernés

- `Tous les endpoints publics stables documentés dans OpenAPI v1.0.0`
...
### BL-API-100-001 — Valider dépendances stables

**Objectif :** Installer et tester core/database/security v1.0.0 exactes ou plages compatibles documentées.
...
### BL-API-100-002 — Valider downstream stable

**Objectif :** Exécuter tests contractuels avec client/admin/service v1.0.0.
...
### BL-API-100-003 — Finaliser documentation

**Objectif :** README, docs endpoints, security, deployment, compatibility, migration, troubleshooting.
...
### BL-API-100-004 — Finaliser release

**Objectif :** Mettre à jour version, changelog, tags, badges, publication package.
...
### BL-API-100-005 — Définition de terminé

**Objectif :** Valider tous les critères fonctionnels, sécurité, qualité et intégration avant tag.
...
- Couverture globale >=90 %, idéalement >=95 % sur services critiques.
- `ruff`, `mypy`, `pytest` passent.
- Tests d’intégration PostgreSQL passent.
- Tests contractuels client v1.0.0 passent.
- Tests d’assemblage service v1.0.0 passent.
- Aucun secret détecté dans logs, erreurs, réponses ou audit.
- OpenAPI v1.0.0 générée et publiée.
...
La version `v1.0.0` est acceptée si :
...
Après `v1.0.0`, toute rupture de contrat devra déclencher une version majeure ultérieure. Les versions mineures pourront ajouter des endpoints compatibles, MFA, OAuth/OIDC avancé ou fonctionnalités d’administration étendues.
