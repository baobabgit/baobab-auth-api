# Cahier des charges — `baobab-auth-api` — v0.9.0

**Projet :** `baobab-auth-api`  
**Version cible :** `v0.9.0`  
**Titre :** Release candidate, non-régression globale et gel des contrats publics  
...
Consolider l’ensemble avant `v1.0.0`. Aucun nouveau périmètre fonctionnel majeur ne doit être ajouté. La version sert à corriger, documenter, mesurer, vérifier la compatibilité et figer les contrats publics.
...
Les dépendances directes de `baobab-auth-api` sont `baobab-auth-core`, `baobab-auth-database` et `baobab-auth-security`. La version `v0.9.0` de l’API doit valider explicitement les versions suivantes avant d’être considérée comme terminée.

| Librairie dépendante directe | Version à valider | Pourquoi cette validation est obligatoire |
|---|---:|---|
| `baobab-auth-core` | `v0.4.0 à v0.9.x` | Revalider toutes les intégrations avec la dernière version pré-1.0 disponible du core sans breaking change non documenté. |
| `baobab-auth-database` | `v0.9.0` | Valider migrations complètes, rollback, index, repositories, UoW et compat PostgreSQL. |
| `baobab-auth-security` | `v0.9.0` | Valider cryptographie, JWKS, rotation, révocation, absence de secrets, performances minimales. |
...
|---|---:|---|
| `baobab-auth-client` | `v0.9.0` | Validation contractuelle complète avant stabilisation 1.0. |
| `baobab-auth-admin` | `v0.9.0` | Validation des workflows admin qui appellent API ou partagent les contrats. |
| `baobab-auth-service` | `v0.9.0` | Validation packaging et déploiement pré-1.0. |

Règle de poursuite : une librairie dépendante ne doit pas poursuivre vers son jalon suivant si l’intégration prévue avec `baobab-auth-api` `v0.9.0` échoue sur un contrat bloquant.
...
- Gel des contrats HTTP publics.
- Matrice de compatibilité complète.
- Tests de non-régression multi-versions si possible.
- Audit sécurité interne : secrets, erreurs, permissions, logs, CORS, rate limit.
- Revue OpenAPI complète.
- Documentation finale en version release candidate.

## 4. Endpoints concernés

- `Tous les endpoints publics introduits de v0.1.0 à v0.8.0`
...
### BL-API-090-001 — Geler les contrats

**Objectif :** Lister tous les endpoints, payloads, erreurs publiques et marquer les breaking changes interdits jusqu’à 1.0.
...
### BL-API-090-002 — Matrice compatibilité

**Objectif :** Créer `docs/compatibility.md` avec versions core/database/security/client/admin/service validées.
...
### BL-API-090-003 — Non-régression complète

**Objectif :** Exécuter unit, api, integration, contract, security, architecture tests.
...
### BL-API-090-004 — Audit sécurité

**Objectif :** Vérifier secrets, permissions, CORS, rate limiting, JWT, refresh rotation, JWKS.
...
### BL-API-090-005 — Documentation RC

**Objectif :** Compléter README, changelog, migration guide, deployment guide.
...
- Tous les tests des versions précédentes passent.
- Aucune réponse ne contient de secret connu ou pattern interdit.
- OpenAPI est stable et versionnée.
- Les tests contractuels client couvrent login/refresh/logout/me/jwks/rbac/admin minimal.
- Les migrations DB de zéro vers état courant fonctionnent.
...
La version `v0.9.0` est acceptée si :
...
La version suivante `v1.0.0` ne doit accepter que des corrections bloquantes, de la documentation finale et la validation des dépendances stables `1.0.0`.
