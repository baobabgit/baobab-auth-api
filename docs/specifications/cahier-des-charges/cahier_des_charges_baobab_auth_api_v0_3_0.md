# Cahier des charges — `baobab-auth-api` — v0.3.0

**Projet :** `baobab-auth-api`  
**Version cible :** `v0.3.0`  
**Titre :** RBAC HTTP, autorisation applicative et contrats rôles/permissions  
...
Transformer le catalogue RBAC en contrat HTTP exploitable. L’API doit exposer clairement les rôles, permissions et droits effectifs, et fournir des dépendances internes d’autorisation pour protéger les routes admin futures.
...
Les dépendances directes de `baobab-auth-api` sont `baobab-auth-core`, `baobab-auth-database` et `baobab-auth-security`. La version `v0.3.0` de l’API doit valider explicitement les versions suivantes avant d’être considérée comme terminée.

| Librairie dépendante directe | Version à valider | Pourquoi cette validation est obligatoire |
|---|---:|---|
| `baobab-auth-core` | `v0.4.0` | Valider `DefaultAuthCatalog`, rôles USER/ADMIN/SERVICE/SUPER_ADMIN, permissions `auth:*`, `AuthorizationService`, règles `SUPER_ADMIN`. |
| `baobab-auth-database` | `v0.3.0` | Valider initialisation catalogue RBAC, repositories rôles/permissions, associations user_roles/role_permissions. |
| `baobab-auth-security` | `v0.2.0` | Valider transport des rôles et permissions dans les claims JWT sans recalcul RBAC. |
...
|---|---:|---|
| `baobab-auth-client` | `v0.2.0` | Valider `require_role`, `require_permission`, parsing roles/permissions depuis JWT et `/auth/roles`. |

Règle de poursuite : une librairie dépendante ne doit pas poursuivre vers son jalon suivant si l’intégration prévue avec `baobab-auth-api` `v0.3.0` échoue sur un contrat bloquant.
...
- Exposition déterministe du catalogue RBAC.
- Réponse `/auth/me` enrichie avec rôles et permissions effectives.
- Dépendances internes `require_current_user`, `require_permission`, `require_role`, `require_super_admin`.
- Normalisation des erreurs 401/403.
- Contrats OpenAPI documentant les rôles et permissions système.
- Tests contractuels avec `baobab-auth-client` pour valider l’interprétation des claims.
...
### BL-API-030-001 — Créer le service RBAC API

**Objectif :** Créer `RoleService` et `PermissionService` qui consomment core/database sans dupliquer le mapping.
...
### BL-API-030-002 — Exposer rôles et permissions

**Objectif :** Implémenter `/auth/roles` et `/auth/permissions` avec tri déterministe, descriptions et indicateur système.
...
### BL-API-030-003 — Enrichir `/auth/me`

**Objectif :** Retourner roles, permissions, session_id, token_id si disponibles, sans secret.
...
### BL-API-030-004 — Créer les dépendances d’autorisation internes

**Objectif :** Implémenter guards FastAPI internes sans dépendre de `baobab-auth-client`.
...
### BL-API-030-005 — Normaliser 403

**Objectif :** Convertir PermissionDenied/Forbidden en HTTP 403 avec code stable.
...
### BL-API-030-006 — Tests RBAC contractuels

**Objectif :** Tester USER/ADMIN/SERVICE/SUPER_ADMIN, permissions auth et permissions applicatives.
...
- `/auth/roles` retourne USER, ADMIN, SERVICE, SUPER_ADMIN.
- `/auth/permissions` retourne les 10 permissions `auth:*`.
- `/auth/me` retourne les permissions effectives du token/session.
- Un rôle SERVICE sans permission est accepté.
- Un token sans permission requise produit 403, pas 401.
- ADMIN ne possède pas implicitement `auth:jwk:rotate`.
...
La version `v0.3.0` est acceptée si :
...
La version suivante pourra ajouter audit HTTP, durcissement des erreurs et protection contre les attaques de login/lockout.
