# Cahier des charges — `baobab-auth-api` — v0.8.0

**Projet :** `baobab-auth-api`  
**Version cible :** `v0.8.0`  
**Titre :** Exploitation, observabilité, rate limiting et préparation service Docker  
...
Préparer l’API à une exécution réaliste : configuration d’environnement, CORS/trusted hosts, rate limiting minimal, logs structurés, métriques simples et compatibilité Docker via `baobab-auth-service`.
...
Les dépendances directes de `baobab-auth-api` sont `baobab-auth-core`, `baobab-auth-database` et `baobab-auth-security`. La version `v0.8.0` de l’API doit valider explicitement les versions suivantes avant d’être considérée comme terminée.

| Librairie dépendante directe | Version à valider | Pourquoi cette validation est obligatoire |
|---|---:|---|
| `baobab-auth-core` | `v0.4.0` | Aucune évolution requise ; revalider policies et exceptions. |
| `baobab-auth-database` | `v0.7.0` | Valider pool async, migrations production, health DB, index utiles sessions/audit/users. |
| `baobab-auth-security` | `v0.5.0` | Valider performances hash/JWT/JWKS et gestion key loading runtime. |
...
|---|---:|---|
| `baobab-auth-service` | `v0.1.0` | Validation d’assemblage Docker dev : API + DB + variables d’environnement. |
| `baobab-auth-client` | `v0.5.0` | Revalidation dans un environnement Docker ou réseau local. |

Règle de poursuite : une librairie dépendante ne doit pas poursuivre vers son jalon suivant si l’intégration prévue avec `baobab-auth-api` `v0.8.0` échoue sur un contrat bloquant.
...
- Configuration production/dev/test claire.
- CORS configurable et désactivé par défaut hors configuration explicite.
- Trusted hosts configurables.
- Rate limiting minimal sur login/register/refresh selon backend choisi ou abstraction.
- Logs structurés sans secret.
- Endpoint `/metrics` optionnel non sensible ou documentation d’intégration future Prometheus.
- Compatibilité avec Dockerfile/service sans embarquer l’assemblage dans la librairie.
...
### BL-API-080-001 — Durcir configuration runtime

**Objectif :** Valider settings obligatoires, secrets absents des logs, profils dev/test/prod.
...
### BL-API-080-002 — Ajouter middlewares sécurité

**Objectif :** CORS, trusted hosts, request id, headers non sensibles.
...
### BL-API-080-003 — Ajouter rate limiting

**Objectif :** Limiter login/register/refresh avec interface désactivable en test.
...
### BL-API-080-004 — Observabilité

**Objectif :** Logs structurés, request id, compteurs simples, documentation.
...
### BL-API-080-005 — Validation service

**Objectif :** Fournir une documentation permettant à `baobab-auth-service` de démarrer l’API.
...
- CORS refuse par défaut une origine non configurée.
- TrustedHost bloque un host non autorisé si configuré.
- Rate limiting bloque les tentatives répétées selon seuil de test.
- Les logs d’erreurs ne contiennent aucun token ou secret.
- `/ready` détecte DB KO et clé active absente.
- Docker/service de test peut démarrer l’API avec env vars.
...
La version `v0.8.0` est acceptée si :
...
La version suivante sera une release candidate dédiée à la compatibilité, aux tests de charge raisonnables, à l’audit sécurité et aux corrections finales.
