# Cahier des charges — `baobab-auth-api` — v0.5.0

**Projet :** `baobab-auth-api`  
**Version cible :** `v0.5.0`  
**Titre :** JWKS avancé, rotation de clés et validation cryptographique contractuelle  
...
Stabiliser l’exposition JWKS et préparer la rotation de clés. L’API doit garantir que les tokens émis peuvent être validés par les APIs consommatrices via JWKS, y compris en présence de rotation contrôlée.
...
Les dépendances directes de `baobab-auth-api` sont `baobab-auth-core`, `baobab-auth-database` et `baobab-auth-security`. La version `v0.5.0` de l’API doit valider explicitement les versions suivantes avant d’être considérée comme terminée.

| Librairie dépendante directe | Version à valider | Pourquoi cette validation est obligatoire |
|---|---:|---|
| `baobab-auth-core` | `v0.4.0` | Valider événement `JWK_ROTATION_REQUESTED`, permissions `auth:jwk:read` et `auth:jwk:rotate`. |
| `baobab-auth-database` | `v0.5.0` | Valider persistance des clés JWK, statuts active/retired/revoked, historique et sélection par kid. |
| `baobab-auth-security` | `v0.4.0` | Valider `JwksProvider`, `KeyRotationService`, `KeyRotationResult`, `kid`, clés publiques et absence de clé privée en sortie. |
...
|---|---:|---|
| `baobab-auth-client` | `v0.3.0` | Valider refresh JWKS, gestion kid inconnu, cache et rejet signature invalide. |

Règle de poursuite : une librairie dépendante ne doit pas poursuivre vers son jalon suivant si l’intégration prévue avec `baobab-auth-api` `v0.5.0` échoue sur un contrat bloquant.
...
- JWKS public conforme, sans clé privée.
- Sélection de la clé active pour signature.
- Conservation des clés retraitées tant que des tokens signés peuvent être valides.
- Endpoint de metadata non sensible des clés.
- Rotation protégée par `auth:jwk:rotate` ou `SUPER_ADMIN`.
- Audit de rotation avec `JWK_ROTATION_REQUESTED` et résultat technique sans secret.

## 4. Endpoints concernés

- `GET /auth/jwks`
- `GET /auth/jwks/metadata`
...
### BL-API-050-001 — Stabiliser `/auth/jwks`

**Objectif :** Publier uniquement kty, kid, use, alg, n/e ou équivalent public selon algorithme.
...
### BL-API-050-002 — Créer metadata JWKS

**Objectif :** Retourner clé active, dates, statuts non sensibles, algorithmes supportés.
...
### BL-API-050-003 — Créer rotation protégée

**Objectif :** Brancher `KeyRotationService`, vérifier permission, auditer, retourner résultat sans secret.
...
### BL-API-050-004 — Compat client

**Objectif :** Tester que `baobab-auth-client` valide un token avant/après rotation et refresh JWKS sur kid inconnu.
...
### BL-API-050-005 — Readiness cryptographique

**Objectif :** Ready doit échouer si aucune clé active ne peut signer.
...
- Chaque token émis contient un `kid` connu du JWKS.
- Après rotation, ancienne clé reste disponible tant que nécessaire.
- Un utilisateur sans `auth:jwk:rotate` reçoit 403 sur rotation.
- La rotation produit un audit sans clé privée.
- Le client valide un token signé avant et après rotation.
...
La version `v0.5.0` est acceptée si :
...
La version suivante pourra exposer des endpoints d’administration utilisateurs/sessions sécurisés en s’appuyant sur RBAC et audit.
